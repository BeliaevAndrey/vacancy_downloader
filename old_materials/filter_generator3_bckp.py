...
def generate_filter(filters: dict[str, str | None]) -> dict[str, Any]:
    expected = {"$and": [{"$and": [{"f_offering_city": {"id": {"$eq": 19}}}]},
...
def choose_gender() -> list[str]:
    ...
    match gender:
        case "М":
            return list(swap(GENDERS)["мужчина"])
        case "Ж":
            return list(swap(GENDERS)["женщина"])
...
def choose_rate() -> list[str]:
    ...
    match rate:
        case "1":
            return list(swap(OFFERING_RATE)["Выработка"])
        case "2":
            return list(swap(OFFERING_RATE)["Фикс"])
    ...
    while True:
        ...
        match filter_num:
            case 0:
                break
            case 1:
                filters["createdAt"] = get_date_time()
            case 2:
                filters["updatedAt"] = get_date_time()
            case 3:
                filters["f_min_age"] = get_int()
            case 4:
                filters["f_offering_max_age"] = get_int()
            case 5:
                filters["f_offering_gender"] = choose_gender()
            case 6:
                filters["f_offering_min_price"] = get_int()
            case 7:
                filters["f_offering_max_price"] = get_int()
            case 8:
                filters["f_offering_men_needed"] = get_int()
            case 9:
                filters["f_offering_women_needed"] = get_int()
            case 10:
